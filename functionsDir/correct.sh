...
while (`echo "$x $MAXTRIALS" | awk '{if ($1 < $2) print 1; else print 0;}'` == 1)
	...
	if (`echo "$CURRENTSUM $TOLERANCE" | awk '{if ($1 < $2) print 1}'` == 1) then
		...
		if (`echo "$CURRENTSUM $TOLERANCE" | awk '{if ($1 < $2) print 1}'` == 1 && `echo "$x $MAXTRIALS" | awk '{if ($1 == $2) print 1; else print 0;}'` == 1) then
			echo "Did not reach Tolerance goal, Current CHI2DOF total: $CURRENTSUM - Tolerance: $TOLERANCE"
			exit
		endif
	endif
